...
(defmulti compile-attrs-kv (fn [name value] name))

(defmethod compile-attrs-kv :class [name value]
  (cond (or (nil? value)
            (keyword? value)
            (string? value))
        value
...
(defmethod compile-attrs-kv :style [name value]
  (util/camel-case-keys value))

(defmethod compile-attrs-kv :default [name value]
  value)
                 ...
                 (assoc m
                   (case k
                     ...
                     (if ((:camelcase-key-pred *config*) k)
                       (util/camel-case k)
                       k))
                   (compile-attrs-kv k v))) {} attrs)
    attrs))
#_(compile-attrs {:class ["b" 'c] :style {:border-width "2px"}}) ;; camelcase style
;; React native style:
#_(compile-attrs {:class ["b" 'c] :style [{:border-width "2px"}]}) ;; camelcase style
#_(compile-attrs {:on-click ()})
...
(defmethod compile-jsx-form :vector [xs]
  (if (util/element? xs)
    (compile-jsx-form-element xs)
    (compile-jsx-form (seq xs))))

(defmethod compile-jsx-form :seq [xs]
  (mapv compile-jsx-form xs))

(defmethod compile-jsx-form :default [x] x)
  ...
  (let [{:keys [transform-fn emit-fn create-element array-children? server-render?]} *config*
        ...
        children (if (and array-children?
                   ...
                   ;; Though, in debug builds of react this will warn about "no keys".
                     ...
                     children)
          el     (tag->el tag)
          attrs  (if server-render? attrs (to-js (compile-attrs attrs)))]
    (if emit-fn
